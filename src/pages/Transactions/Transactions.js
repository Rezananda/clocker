...
const Transactions = () => {
  const [historyData, historyEmpty, initializeHistory, scroll, getHistory] = useGetHistory()
  ...
  return (
    <>
      <div className='flex flex-col sticky top-0'>
        <TopNavbar navbarColor={`bg-blue-500`} label={`Riwayat`} labelColor={`text-white`} navigateTo={false}/>
        <style>
            {
        ...
        <div className='flex items-center gap-2 px-4 overflow-x-auto scrollable mb-2 mt-4'>
          ...
          <Chip text={"Kehadiran"} enable={filter === 'attendance'} handleClick={() => handleFilter('attendance')} /> 
          <Chip text={"Bergabung Grup"} enable={filter === 'join group'} handleClick={() => handleFilter('join group')} /> 
          <Chip text={"Buat Grup"} enable={filter === 'add group'} handleClick={() => handleFilter('add group')} /> 
          <Chip text={"Menyetujui Grup"} enable={filter === 'approve group'} handleClick={() => handleFilter('approve group')} /> 
        </div>
      </div>
        {initializeHistory? 
        <LoadingTransaction/>
        ...
        <div className='flex flex-col overflow-y-auto h-screen px-4' id="scrollableDiv">
          ...
          <p className='flex items-center justify-center text-sm text-gray-500'>-Belum Ada Riwayat-</p>
          :
          <>
          <ul>
            <InfiniteScroll
                ...
                <div className='px-4 py-2 flex items-center justify-between'>
                  <div className='flex items-center gap-2'>
                    ...
                    <div className='flex flex-col dark:text-white'>
                      ...
                      {val.transaction === "not approve grup"&&<p className='font-bold'>Ditolak Grup</p>}
                      {val.transaction === 'decline group'&&<p className='font-bold'>Menolak </p>}
                      {val.transactionType === "add"&&<div className='bg-blue-100 w-fit text-xs text-blue-500 rounded py-0.5 px-1 font-bold'>Tambah</div>}
                      {val.transactionType === "update"&&<div className='bg-orange-100 w-fit text-xs text-orange-500 rounded py-0.5 px-1 font-bold'>Ubah</div>}
                  </div>
                  </div>
                  <p className='text-xs dark:text-white'>{moment(val.date.seconds * 1000).format('DD/MM/YYYY')} </p>
                </div>
              </li>
            ))}
          </InfiniteScroll>
