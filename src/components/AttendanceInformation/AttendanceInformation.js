            ...
            <>
                ...
                <div className='flex flex-col gap-2 p-4'>
                    ...
                    <div className='flex items-center justify-between'>
                        ...
                        <div className='text-blue-500 font-bold px-2 py-1 rounded flex gap-2 font-bold bg-blue-100'>
                            {personalAttendance.find(val => val.addDate === new Date(Timestamp.now().seconds*1000).toLocaleDateString()).status} 
                            {personalAttendance.find(val => val.addDate === new Date(Timestamp.now().seconds*1000).toLocaleDateString()).status === 'WFO' && '🏢'}
                            {personalAttendance.find(val => val.addDate === new Date(Timestamp.now().seconds*1000).toLocaleDateString()).status === 'WFH' && '🏠'}
                            {personalAttendance.find(val => val.addDate === new Date(Timestamp.now().seconds*1000).toLocaleDateString()).status === 'Cuti' && '🏖️'}
                            {personalAttendance.find(val => val.addDate === new Date(Timestamp.now().seconds*1000).toLocaleDateString()).status === 'Sakit' && '😷'}
                            <ButtonIcon icon={
