...
const GroupAttendanceInformation = () => {
  ...
  return (
    <div className='px-4'>
        <p className='font-bold'>Kehadiran Hari Ini ({new Date().getDate()}/{new Date().getMonth()+1})</p>
        ...
        <div>
          ...
          <div className='flex w-full gap-1 overflow-x-auto scrollable sticky top-0 bg-gray-100 py-2'>
            <Chip text="Sudah Isi" enable={filter === 'all'} isCount={true} count={allAttendance.filter(item => item.addDate === new Date(Date.now()).toLocaleDateString()).length} handleClick={() => handleFilter('all')} color={'blue'} /> 
            ...
            <Chip text="Belum Isi" handleClick={() => handleFilter('all')} />             
          </div>
          {attendanceInfo === 'noAttendance' &&<p className='text-sm text-gray-500 text-center'>-Belum Ada Kehadiran-</p>}

          <div className='flex flex-col gap-1'>
          {attendanceInfo !== "noGroup" && attendanceInfo !== "noAttendance"&&attendanceInfo.filter(val => val.addDate === new Date(Timestamp.now().seconds*1000).toLocaleDateString()).map((val, index) => (
          ...
          {initializeAttendanceMore&&
            ...
            {!initializeAttendanceMore&&!attendanceEmpty&&attendanceInfo !== "noGroup" && attendanceInfo !== "noAttendance"&&<ButtonLink linkTo={() => scroll(filter === 'all' ? 'all' : filter === 'WFO' ? 'wfo' : filter === 'WFH' ? 'wfh' : filter === 'Cuti' ? 'cuti' : '' )} newProps={'text-sm'} label={'Selanjutnya'}/>}
          </div>
        </div>
        :
        ""
